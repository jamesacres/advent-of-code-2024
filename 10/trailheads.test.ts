...
Deno.test(function exampleFindTrailheadsV2() {
  const result = toTrailHeadsArray(findTrailheadRoutes(example, true));
  assertEquals(
    result,
    [
      {
...
Deno.test(function exampleSumTrailheadsV2() {
  const result = toTrailHeadsArray(findTrailheadRoutes(example, true)).reduce(
    (result, { trailCount }) => result + (trailCount || 0),
    0,
  );
  assertEquals(
...
Deno.test(async function inputSumTrailheads() {
  const input = await Deno.readTextFile("./10/input.txt");
  const result = toTrailHeadsArray(findTrailheadRoutes(input, true)).reduce(
    (result, { trailCount }) => result + (trailCount || 0),
    0,
  );
  assertEquals(
